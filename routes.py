...
def add_ksiazka():
    ...
    try:
        ...
        db.session.commit()
        return jsonify({"message": "Ksiazka dodana pomyslnie"}), 201
    except Exception as e:
        db.session.rollback()
        return jsonify({"error": str(e)}), 400
...
def update_ksiazka(id):
    ...
    try:
        ...
        db.session.commit()
        return jsonify({"message": "Ksiazka zaktualizowana"}), 200
    except Exception as e:
        db.session.rollback()
        return jsonify({"error": str(e)}), 400
...
def create_rezerwacja():
    ...
    try:
        ...
        if not egzemplarz:
            return jsonify({"error": "Brak dostępnych egzemplarzy do rezerwacji"}), 400
